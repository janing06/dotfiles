...
- Write/update tests first before implementation when tests are necessary (skip for super trivial cases)
...
4. **Testability** - Can I easily test this? (if tests already exist or if it would be easy to add tests for it in the future)
...
**NEVER**:
- Commit automatically — always wait for the user to invoke `/commit`
- Bulk-commit all changes in one go — commit each logical change separately
